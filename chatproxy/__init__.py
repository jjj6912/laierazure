...
def search_ctx(vs,q):
    sr = requests.post(f"{EP}/openai/vectorstores/{vs}/similarity_search?api-version=2025-01-01-preview",
                       headers=HDR,json={"query":q,"top_k":4}).json()
...
def main(req: func.HttpRequest):
    ...
    if not inc(user):
        return func.HttpResponse(json.dumps({"reply":"Límite 600/mes alcanzado"}),403,mimetype="application/json")

    vs=b.get("vs_id")
    q = b["message"]
    msgs=[{"role":"system","content":"Eres LAIER, asistente legal experto en derecho español."}]
    if vs: msgs.append({"role":"system","content":"Contexto:\n"+search_ctx(vs,q)})
    msgs.append({"role":"user","content":q})

    r = requests.post(f"{EP}/openai/deployments/{DEP}/chat/completions?api-version=2025-01-01-preview",
                      headers=HDR,json={"messages":msgs,"max_tokens":800}).json()
    ans=r["choices"][0]["message"]["content"]
    return func.HttpResponse(json.dumps({"reply":ans,"vs_id":vs}),mimetype="application/json")
